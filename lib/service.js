...
Goblin.registerQuest(goblinName, 'open-session', function* (
  ...
  const mainConfig = require('xcraft-core-etc')().load(`goblin-${mainGoblin}`);
  if (mainConfig && mainConfig.profile && mainConfig.profile.mandate) {
    mandate = mainConfig.profile.mandate;
  }
