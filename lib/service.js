...
Goblin.registerQuest(goblinName, 'start', function*(quest) {
  ...
  const feedList = [labId, clientConfig.mainGoblin, 'client'];
  if (quest.hasAPI('nabu')) {
    feedList.push('nabu');
  }
  if (quest.hasAPI('workshop')) {
    feedList.push('workshop');
  }
  const config = {
    feeds: feedList,
    useWS,
  ...
  quest.goblin.defer(
    quest.sub(`*::*.user-locale-changed`, function*(_, {msg, resp}) {
      const {data} = msg;
      yield resp.cmd(`${goblinName}.change-locale`, data);
    })
  );
...
Goblin.registerQuest(goblinName, 'load-laboratory', function*(quest) {
  ...
  if (process.versions.electron) {
    ...
    quest.dispatch('set-locale', {locale});
  }
...
Goblin.registerQuest(goblinName, 'configure-desktop', function*(
  ...
  if (_data.localeName) {
    quest.dispatch('set-locale', {locale: _data.localeName});
  }
...
Goblin.registerQuest(goblinName, 'set-lang', function(quest, lang) {
  const locale = quest.goblin.getState().get('locale');
  if (!locale) {
    quest.do({lang});
  }
});
