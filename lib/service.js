...
Goblin.registerQuest(goblinName, 'boot', function* (quest, $msg, next) {
  ...
  if (useDevTools) {
    try {
      ...
      for (const extension of devtoolsInstalled) {
        const manifest = extension.manifest;
        if (
          manifest.manifest_version === 3 &&
          manifest?.background?.service_worker?.startWorkerForScope
        ) {
          yield sessionToUse.serviceWorkers.startWorkerForScope(extension.url);
        }
      }
  ...
  if (clientConfig.useLogin) {
    ...
    switch (status) {
      ...
      default: {
        ...
        const wm = quest.getAPI(`wm@${labId}`);
        yield wm.setUserId({userId});
      }
    }
  }
...
Goblin.registerQuest(goblinName, 'open-external', async function (quest, url) {
  ...
  try {
    ...
    quest.log.err(
      `An error occurred with method shell.${method} and url ${url}`,
      err.stack || err.message || err
    );
  } finally {
