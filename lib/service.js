...
Goblin.registerQuest(goblinName, 'start', function*(quest) {
  ...
  quest.goblin.defer(
    quest.sub(`${clientConfig.mainGoblin}.desktop.closed`, function*(_, {msg}) {
      const {data} = msg;
      if (clientConfig.useConfigurator) {
        yield quest.me.configure({clientConfig, oldDesktopId: data.desktopId});
      }
    })
  );
...
Goblin.registerQuest(goblinName, 'configure-desktop', function*(
  ...
  const mandate = configuration ? configuration.mandate : undefined;
  ...
  yield lab.setRoot({widgetId: _data.desktopId});
...
Goblin.registerQuest(goblinName, 'configure', function*(
  ...
  quest.goblin.setX(
    'confUnsub',
    quest.sub(`${conf.id}.configured`, function*(err, {msg}) {
      yield quest.me.configureDesktop({clientConfig, labId, data: msg.data});
    })
  );
