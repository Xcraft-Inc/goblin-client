...
Goblin.registerQuest(goblinName, 'boot', function* (quest, $msg, next) {
  ...
  if (useDevTools) {
    try {
      ...
      try {
        // TODO: temporary, to be removed when electron will support manifest v3 (see https://github.com/facebook/react/issues/25843)
        const {installCustomExtensionVersion} = require('./dev-extensions.js');
        const LEGACY_REACT_DEVELOPER_TOOLS_URL =
          'https://owncloud.epsitec.ch/owncloud/index.php/s/79HH4sfKeFLt39i/download/ReactDevTools-4.27.1_manv2.zip';

        yield installCustomExtensionVersion(
          REACT_DEVELOPER_TOOLS,
          LEGACY_REACT_DEVELOPER_TOOLS_URL,
          {},
          next
        );
      } catch (err) {
        quest.log.warn(
          `An error occurred installing REACT_DEVELOPER_TOOLS extension: `,
          err.stack || err
        );
      }
      ...
      try {
        ...
        quest.log.warn(
          `An error occurred installing REDUX_DEVTOOLS extension: `,
          err.stack || err
        );
      }
...
Goblin.registerQuest(goblinName, 'start', function* (quest, $msg, next) {
  const clientConfig = require('xcraft-core-etc')().load('goblin-client');
...
Goblin.registerQuest(goblinName, 'createSession', function* (
  ...
  let files = fse.readdirSync(sessionStorage);
  let clientSessionId;
...
Goblin.registerQuest(goblinName, 'logout', function* (quest) {
  ...
  if (fse.existsSync(passportRootStorage)) {
    const tokenFile = path.join(passportRootStorage, 'refresh.tkn');
    if (fse.existsSync(tokenFile)) {
      fse.removeSync(tokenFile);
    }
  }
  const loginSessionId = quest.goblin.getX('loginSessionId');
  const loginSessionAPI = quest.getAPI(loginSessionId);
  const tokenData = yield loginSessionAPI.deleteTokens();
  if (tokenData) {
...
Goblin.registerQuest(goblinName, 'login', function* (
  ...
  const passportRootStorage = path.join(xConfig.xcraftRoot, 'var/passport');
  mkdir(passportRootStorage);
  const tokenFile = path.join(passportRootStorage, 'refresh.tkn');
  const refreshTokenExist = fse.existsSync(tokenFile);

  let mustRenew = true;
  let token;
  if (refreshTokenExist) {
    const refreshToken = fse.readFileSync(tokenFile);
    try {
  ...
  if (mustRenew) {
    ...
    switch (token.status) {
      case 'renewed': {
        fse.writeFileSync(tokenFile, token.refreshToken);
      }
    }
  }
  const info = yield hubAPI.getUserData();
  const accessToken = yield hubAPI.getAccessTokenData();
  if (accessToken) {
...
Goblin.registerQuest(goblinName, 'configure', function* (
  ...
  if (desktops) {
    for (const desktopId of desktops) {
      conf.openSession({feed: desktopId}, next.parallel());
    }
    yield next.sync();
