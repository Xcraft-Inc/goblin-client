...
Goblin.registerQuest(goblinName, 'boot', function* (quest, $msg, next) {
  ...
  app.getGPUInfo('basic').then((/* infos */) => {
    /* We use getGPUInfo promise in order to be sure that gpu-info-update was emitted */
  ...
  let ignoreCloseRequests = false;

  app.on('window-all-closed', () => {
    if (ignoreCloseRequests) {
      quest.log.dbg(
        'Ignore close app request while the bus client is reconnecting'
      );
      return;
    }
...
Goblin.registerQuest(goblinName, 'start', function* (quest, $msg, next) {
  ...
  quest.warehouse.feedSubscriptionAdd(
    {
      feed: labId,
      branch: goblinOrcId,
      parents: 'goblin',
