...
const logicHandlers = {
  ...
  'onReconnect': (state, action) => {
    return state.set(
      `private.isDisconnected`,
      action.get('status') === 'attempt'
    );
  },
};
...
Goblin.registerQuest(goblinName, 'start', function* (
  ...
  quest.goblin.defer(
    quest.resp.onReconnect(
      watt(function* (status) {
        switch (status) {
          case 'attempt':
          case 'done':
            yield quest.me.onReconnect({status});
            break;
        }
      })
    )
...
Goblin.registerQuest(goblinName, 'onReconnect', function (quest, status) {
  quest.do();
  switch (status) {
    case 'attempt':
      quest.log.warn('Connection lost with the server, attempt a reconnection');
      break;
    case 'done':
