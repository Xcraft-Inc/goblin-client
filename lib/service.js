...
Goblin.registerQuest(goblinName, 'load-laboratory', function* (
  ...
  const config = {
    feeds: getBaseFeeds(labId, clientConfig.mainGoblin),
    useWS,
    target,
...
Goblin.registerQuest(goblinName, 'configure-desktop', function* (
  ...
  let themeContext = clientConfig.themeContexts[0];
...
Goblin.registerQuest(goblinName, 'open-session', function* (
  ...
  config.feeds = getBaseFeeds(labId, mainGoblin || clientConfig.mainGoblin);
  ...
  if (!mainGoblin) {
    ...
    mainGoblinAPI = quest.getAPI(clientConfig.mainGoblin);
  } else {
    //resolve to API
...
Goblin.registerQuest(goblinName, 'open-window', function* (
  ...
  config.feeds = getBaseFeeds(labId, clientConfig.mainGoblin);
