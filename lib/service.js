...
/*Goblin.registerQuest(goblinName, 'set-bounds', function*(quest, bounds) {
  const sessionId = quest.goblin.getX('clientSessionId');
  if (sessionId) {
    const clientSessionApi = quest.getAPI(sessionId);
    yield clientSessionApi.setBounds({bounds});
  }
});*/
