...
Goblin.registerQuest(goblinName, 'create', function(
  ...
  const globalSessionPath = path.join(sessionStorage, `${id}-global`);
  const appSessionPath = path.join(sessionStorage, `${id}-${app}`);

  const globalSettings = new StateDb(globalSessionPath, 'global');
  const appSettings = new StateDb(appSessionPath, app);
