...
const logicHandlers = {
  ...
  'set-window-state': (state, action) => {
    ...
    // Initialization of a new window
    if (winIndex === null) {
      // New window index
      winIndex = state.get(`private.windowIndexes.count`);
      // Set window index for this window id
      state = state.set(`private.windowIndexes.${winId}`, winIndex);
      // Get existing window state at current count
      const storedWindowState = state.get(`windows[${winIndex}]`);
      // If stored window state doesn't exist
      // first time app launched or client-session db empty/deleted
      if (!storedWindowState) {
        // Set default state for the new window
        state.log.dbg(`new window ${winId} set default state ${winIndex}`);
        state = state.set(`windows[${winIndex}]`, action.get('state'));
      } else {
        state.log.dbg(`new window ${winId} restored state ${winIndex}`);
      }
      //increment number of windows
      state = state.set(`private.windowIndexes.count`, winIndex + 1);
    } else {
...
Goblin.registerQuest(goblinName, 'create', function* (
  ...
  let session = null;
  try {
    session = settings.loadState('session');
  } catch (ex) {
    quest.log.warn(
      `error loading client session state: ${ex.stack || ex.message || ex}`
    );
  }
...
Goblin.registerQuest(goblinName, 'get-window-state', function* (
  ...
  let windowState;
  if (winIndex !== null) {
    windowState = state.get(`windows[${winIndex}]`);
  } else {
  ...
  if (!windowState) {
    windowState = new Goblin.Shredder(defaultState);
  }
  return windowState;
});
...
Goblin.registerQuest(goblinName, 'save', function (quest) {
  ...
  try {
    ...
    quest.log.warn(
      `error saving client session state: ${ex.stack || ex.message || ex}`
    );
  }
});
